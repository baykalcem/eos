...
class TestCpSatScheduler:
    ...
    async def test_register_experiment(self, cp_sat_scheduler, experiment_graph):
        await cp_sat_scheduler.register_experiment("exp1", "abstract_experiment", experiment_graph)
        assert cp_sat_scheduler._registered_experiments["exp1"] == (
            "abstract_experiment",
            experiment_graph,
        )

    @pytest.mark.asyncio
    async def test_register_invalid_experiment(self, cp_sat_scheduler, experiment_graph):
        with pytest.raises(EosSchedulerRegistrationError):
            await cp_sat_scheduler.register_experiment("exp1", "invalid_type", experiment_graph)

    @pytest.mark.asyncio
    async def test_unregister_experiment(self, db, cp_sat_scheduler, experiment_graph, setup_lab_experiment):
        # Register experiment
        await cp_sat_scheduler.register_experiment("exp1", "abstract_experiment", experiment_graph)
        assert "exp1" in cp_sat_scheduler._registered_experiments

        # Unregister experiment
        await cp_sat_scheduler.unregister_experiment(db, "exp1")
        assert "exp1" not in cp_sat_scheduler._registered_experiments

    @pytest.mark.asyncio
    async def test_unregister_nonexistent_experiment(self, db, cp_sat_scheduler):
        with pytest.raises(EosSchedulerRegistrationError):
            await cp_sat_scheduler.unregister_experiment(db, "nonexistent")

    @pytest.mark.asyncio
    async def test_request_tasks_unregistered_experiment(self, db, cp_sat_scheduler):
        with pytest.raises(EosSchedulerRegistrationError):
            await cp_sat_scheduler.request_tasks(db, "nonexistent")
    ...
    async def test_correct_schedule(
        ...
        await cp_sat_scheduler.register_experiment("experiment_1", "abstract_experiment", experiment_graph)
        ...
        for expected_batch in scheduling_sequence:
            await self._process_and_verify_tasks(
                db, cp_sat_scheduler, resource_allocation_manager, task_manager, "experiment_1", expected_batch
            )

        # Verify experiment completion
        assert await cp_sat_scheduler.is_experiment_completed(db, "experiment_1")

        # Verify no more tasks are scheduled
        await cp_sat_scheduler.request_tasks(db, "experiment_1")
        await resource_allocation_manager.process_requests(db)
        final_tasks = await cp_sat_scheduler.request_tasks(db, "experiment_1")
