...
class TestCampaignManager:
    @pytest.mark.asyncio
    async def test_create_campaign(self, campaign_manager):
        await campaign_manager.create_campaign(create_campaign_definition("test_campaign"))

        campaign = await campaign_manager.get_campaign("test_campaign")
        assert campaign.id == "test_campaign"
        assert len(campaign.dynamic_parameters) == 2
        assert campaign.metadata == {"test": "metadata"}

    @pytest.mark.asyncio
    async def test_create_campaign_validation_errors(self, campaign_manager):
        ...
        with pytest.raises(ValueError, match="Campaign dynamic parameters must be provided"):
            ...
            await campaign_manager.create_campaign(invalid_definition)
    ...
    @pytest.mark.asyncio
    async def test_create_campaign_nonexistent_type(self, campaign_manager):
        with pytest.raises(EosCampaignStateError):
            ...
            await campaign_manager.create_campaign(definition)

    @pytest.mark.asyncio
    async def test_create_existing_campaign(self, campaign_manager):
        definition = create_campaign_definition("test_campaign")
        await campaign_manager.create_campaign(definition)
    ...
    @pytest.mark.asyncio
    async def test_delete_campaign(self, campaign_manager):
        await campaign_manager.create_campaign(create_campaign_definition("test_campaign"))

        campaign = await campaign_manager.get_campaign("test_campaign")
        assert campaign is not None

        await campaign_manager.delete_campaign("test_campaign")

        campaign = await campaign_manager.get_campaign("test_campaign")
        assert campaign is None

    @pytest.mark.asyncio
    async def test_get_campaigns_by_status(self, campaign_manager):
        # Create and set different statuses for campaigns
        for campaign_id in ["campaign1", "campaign2", "campaign3"]:
            await campaign_manager.create_campaign(create_campaign_definition(campaign_id))

        await campaign_manager.start_campaign("campaign1")
        await campaign_manager.start_campaign("campaign2")
        await campaign_manager.complete_campaign("campaign3")

        running_campaigns = await campaign_manager.get_campaigns(status=CampaignStatus.RUNNING.value)
        completed_campaigns = await campaign_manager.get_campaigns(status=CampaignStatus.COMPLETED.value)
    ...
    @pytest.mark.asyncio
    async def test_campaign_lifecycle(self, campaign_manager):
        await campaign_manager.create_campaign(create_campaign_definition("test_campaign"))
        ...
        await campaign_manager.start_campaign("test_campaign")
        campaign = await campaign_manager.get_campaign("test_campaign")
        assert campaign.status == CampaignStatus.RUNNING
        assert campaign.start_time is not None
        assert campaign.end_time is None

        await campaign_manager.complete_campaign("test_campaign")
        campaign = await campaign_manager.get_campaign("test_campaign")
    ...
    @pytest.mark.asyncio
    async def test_campaign_experiments(self, campaign_manager):
        await campaign_manager.create_campaign(create_campaign_definition("test_campaign"))

        # Test adding experiments
        await campaign_manager.add_campaign_experiment("test_campaign", "exp1")
        await campaign_manager.add_campaign_experiment("test_campaign", "exp2")
        ...
        await campaign_manager.delete_campaign_experiment("test_campaign", "exp1")
        campaign = await campaign_manager.get_campaign("test_campaign")
        assert len(campaign.current_experiment_ids) == 1
        assert "exp2" in campaign.current_experiment_ids

        # Test clearing all experiments
        await campaign_manager.delete_current_campaign_experiments("test_campaign")
        campaign = await campaign_manager.get_campaign("test_campaign")
